...
import { DashboardIcon, InvoiceIcon, CustomerIcon, ProductIcon, LogoutIcon, MenuIcon, CloseIcon } from '../constants';
    ...
    const sidebarContent = (
      <div className="flex h-full flex-col bg-white dark:bg-gray-800 shadow-lg">
          <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h1 className="text-xl font-bold text-primary-600 dark:text-primary-400">Invoicer</h1>
          ...
          <div className="flex flex-col justify-between flex-1">
                              ...
                              <NavLink
                                  ...
                                  className={({ isActive }) =>
                                      `flex items-center px-4 py-3 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 ${
                                          isActive ? 'bg-primary-50 dark:bg-gray-700 border-r-4 border-primary-500 text-primary-600 dark:text-white' : ''
                                      }`
                                  }
                              >
                                  {item.icon}
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="mb-4">
                      <p className="text-sm font-semibold text-gray-800 dark:text-white">{userProfile?.companyName}</p>
    ...
    return (
        <>
            <div className="lg:hidden fixed top-4 left-4 z-20">
              <button onClick={() => setIsOpen(true)} className="p-2 bg-white dark:bg-gray-800 rounded-md shadow-md">
                <MenuIcon />
              </button>
            </div>
            {/* Mobile Sidebar */}
            <div className={`fixed inset-0 z-30 transform transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
              <div className="w-64 h-full">
                {sidebarContent}
              </div>
              <div className="flex-1 bg-black opacity-50" onClick={() => setIsOpen(false)}></div>
            </div>
